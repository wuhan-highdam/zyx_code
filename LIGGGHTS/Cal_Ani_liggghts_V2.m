...
for f = 1:length(fileNames);
    ...
    a_c1 = sign(sum(sum(aFabric_c.*sigema_d)))*sqrt(1.5*sum(sum(aFabric_c.^2)));
    a_fn1 = sign(sum(sum(aFabric_fn.*sigema_d)))*sqrt(1.5*sum(sum(aFabric_fn.^2)));
    a_ft1 = sign(sum(sum(aFabric_ft.*sigema_d)))*sqrt(1.5*sum(sum(aFabric_ft.^2)));
    a_d1 = sign(sum(sum(aFabric_d.*sigema_d)))*sqrt(1.5*sum(sum(aFabric_d.^2)));
